...
def open(
    ...
    if not found_devices:
        ...
        if len(found_devices) > 0:
            ...
            # Prefer the requested index first, then try the rest.
            start_idx = DeviceNumber if 0 <= DeviceNumber < len(found_devices) else 0
            ordered_indices = [start_idx] + [i for i in range(len(found_devices)) if i != start_idx]
            last_exc = None

            for idx in ordered_indices:
                spec = found_devices[idx]["Spec"]
                dev = found_devices[idx]["HIDDevice"]
