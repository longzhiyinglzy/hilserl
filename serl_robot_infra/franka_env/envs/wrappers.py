...
class SpacemouseIntervention(gym.ActionWrapper):
    def __init__(self, env, action_indices=None):
        ...
        self._prev_left_button = 0
        self._prev_right_button = 0
    ...
    def action(self, action: np.ndarray) -> np.ndarray:
        ...
        buttons = list(buttons) + [0, 0]
        left_button = int(buttons[0])
        right_button = int(buttons[1])
        self.left, self.right = bool(left_button), bool(right_button)
        intervened = np.any(np.abs(expert_a) > 0.0)

        if self.gripper_enabled:
            if left_button == 1 and self._prev_left_button == 0:  # open gripper
                gripper_action = np.array([1.0])
                intervened = True
            elif right_button == 1 and self._prev_right_button == 0:  # close gripper
                gripper_action = np.array([-1.0])
                intervened = True
            else:
        ...
        self._prev_left_button = left_button
        self._prev_right_button = right_button
    ...
    def step(self, action):
        ...
        if replaced:
            ...
            info["spacemouse_raw"] = self.expert.latest_raw.copy()
        info["left"] = self.left
...
class DualSpacemouseIntervention(gym.ActionWrapper):
    ...
    def action(self, action: np.ndarray) -> np.ndarray:
        ...
        intervened = False
        expert_a, buttons = self.expert.get_action()
        buttons = list(buttons) + [0, 0, 0, 0]
        self.left1, self.left2, self.right1, self.right2 = tuple(bool(int(x)) for x in buttons[:4])

        if expert_a.shape[0] < 12:
            padded_expert_a = np.zeros((12,), dtype=expert_a.dtype)
            padded_expert_a[: expert_a.shape[0]] = expert_a
            expert_a = padded_expert_a
